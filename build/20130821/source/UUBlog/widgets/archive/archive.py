...
config={}
config.setdefault("name","archive")
config.setdefault("title","归档")
config.setdefault("description","归档")
config.setdefault("initparams",utility.Obj2Json({"width":400,"height":200}))
config.setdefault("initdata","")

class ArchiveSetting(UBaseWidgetView):
   
    
    def __init__(self,*args, **kwargs):
        super(ArchiveSetting, self).__init__(**kwargs)
...
def Archive(request,widget,kwargs={}):
    return widget.data


config.setdefault("setting",ArchiveSetting)
config.setdefault("view",Archive)