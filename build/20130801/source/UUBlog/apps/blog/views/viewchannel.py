...
class IndexView(UBaseBlogView):
    ...
    def GetContext(self, **kwargs):
        ...
        channelList=Channel.objects.filter(parent_id=0,isenable=1)

        channelJson="["

        channel1Array="channel1[0]=["
        channel2Array=""

        channel1Dot=""
        for channel1 in channelList:
            channel1Array+='%s[%s,"%s"]' %(channel1Dot,channel1.id,channel1.name)

            channel2List=Channel.objects.filter(parent_id=channel1.id,isenable=1)
            channel2Array+="<br>channel2[%s]=[" %channel1.id

            channel2Dot=""
            for channel2 in channel2List:
                channel2Array+='%s[%s,"%s"]' %(channel2Dot,channel2.id,channel2.name)
                channel2Dot=","
            channel2Array+="];"

            #channelJson+=r'%s<br>{id:%s,name:"%s",children:%s}' %(channel1Dot,channel1.id,channel2.name,children)
            channel1Dot=","
        ...
        if cid==-1:
            parentChannel=Channel(name="全部")
            childrenChannel=None

            articleList=Article.objects.all().order_by("-createtime")
            #oneWeekViewsArticleList=Article.objects.order_by("-views")[0:10]
            #oneWeekGoodsArticleList=Article.objects.order_by("-goods")[0:10]
        else:
            ...
            try:
                childChannel=Channel.objects.get(parent_id=c2id)
            ...
            #一周热门文章
            #oneWeekViewsArticleList=Article.objects.order_by("-views").filter(channel1_id=cid)[0:10]
            #oneWeekGoodsArticleList=Article.objects.order_by("-goods").filter(channel1_id=cid)[0:10]
