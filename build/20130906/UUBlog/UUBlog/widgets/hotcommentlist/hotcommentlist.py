...
config={}
config.setdefault("name","hotcommentlist")
config.setdefault("title","热门评论")
config.setdefault("description","热门评论")
config.setdefault("initparams",utility.Obj2Json({"width":400,"height":200}))
config.setdefault("initdata","")


class HotCommentListSetting(UBaseWidgetView):
...
def HotCommentListView(request,widget,kwargs={}):
    articleList=Post.objects.order_by("-views").filter(status=1)[:5]
    return articleList



config.setdefault("setting",HotCommentListSetting)
config.setdefault("view",HotCommentListView)
