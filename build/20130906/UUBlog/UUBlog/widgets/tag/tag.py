...
config={}
config.setdefault("name","tag")
config.setdefault("title","tag")
config.setdefault("description","tag")
config.setdefault("initparams",utility.Obj2Json({"width":400,"height":200}))
config.setdefault("initdata","")

class TagSetting(UBaseWidgetView):
...
def TagView(request,widget,kwargs={}):
    articleList=Post.objects.order_by("-views").filter(status=1)[:5]
    return articleList



config.setdefault("setting",TagSetting)
config.setdefault("view",TagView)
